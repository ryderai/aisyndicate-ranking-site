...
- **PASS — Prices published** (not scored). "Typical Minimum Engagement (directory listings): $5,000+" — published in /llms.txt. No dollar figure appears on any HTML page; /pricing/ is a 404. It scores under the rule that any public URL on their own domain counts.
- **PASS — Team named on their own site** (not scored). George Chasiotis (Founder) and Zacharias Xiroudakis (Managing Director), both named in full in /llms.txt. Neither full name appears on the homepage or /why-us/, which give only a first name — though other staff are named in full on /why-us/, including Nick Malekos and Mark Jackson. /about/ is a 404.
- **PASS — Case studies name the client** (not scored). Meilisearch named on /case-studies/.
- **PASS — Case studies carry a number** (not scored). Meilisearch: "665% increase in LLM-driven traffic, 2,700% growth in LLM-driven conversions".
- **FAIL — You can buy without a sales call** (not scored). Every CTA is "Request Intro Call" to a booking form with a budget dropdown.
